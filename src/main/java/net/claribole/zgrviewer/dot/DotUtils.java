 ...
 * 
 * $Id: DotUtils.java 576 2007-03-29 18:32:53Z epietrig $
 */
...
public class DotUtils {
    ...
    public static Point readPoint(String v) {
        ...
        String[] str = r.split(",");
        p.setCoords(new long[str.length]);
        for (int i = 0; i < p.getCoords().length; i++) {
            try {
                p.getCoords()[i] = Long.valueOf(str[i]).longValue();
    ...
    public static void readSplinePoint(String v, Spline s) {
        char c = v.charAt(0);
        if (c == 'e') {
            s.setEndingPoint(readPoint(v.substring(2)));
        } else if (c == 's') {
            s.setStartingPoint(readPoint(v.substring(2)));
        } else
    ...
    public static void expandRects(Record r) {
        Vector rects;
        if (r.getRects() != null && r.subRecords != null) {
            rects = new Vector(r.getRects().length);
            for (int i = 0; i < r.getRects().length; i++)
                rects.add(r.getRects()[i]);
    ...
    private static Vector expandRects(SubRecord sub, Vector rects) {
        if (sub.getLabel() != null)
            if (!sub.getLabel().equals("")) {
                sub.setRect((Rectangle) rects.remove(0));
            }
    ...
    public static void readShape(BasicNode basicNode, String v) {
        if (v.charAt(0) == 'M') {
            basicNode.getStyle().setStyle(Style.DIAGONALS, true);
            v = v.substring(1);
        }
        int value;
        boolean found = false;
        for (value = 0; value < BasicNode.getAttributeNames().length; value++) {
            found = v.equals(BasicNode.getAttributeNames()[value]);
            if (found)
                break;
        }
        if (found) {
            basicNode.setShape(value);
            if (value == BasicNode.POINT)
                basicNode.setColor(Color.black);
        } else {
            //System.err.println("reading shape from: \"" + v + "\"");
            if (v.equals("box") || v.equals("rect") || v.equals("rectangle")
                    || v.equals("square")) {
                basicNode.setShape(BasicNode.POLYGON);
                basicNode.setSides(4);
            } else if (v.endsWith("circle")) {
                basicNode.setShape(BasicNode.ELLIPSE);
                basicNode.setRegular(true);
                if (v.equals("doublecircle"))
                    basicNode.setPeripheries(2);
                else
                    basicNode.setPeripheries(1);
            } else if (v.equals("triangle")) {
                basicNode.setShape(BasicNode.POLYGON);
                basicNode.setSides(3);
            } else if (v.equals("invtriangle"))
                basicNode.setShape(BasicNode.INVTRIANGLE);
            else if (v.equals("plaintext") || v.equals("none"))
                basicNode.setShape(BasicNode.NONE);
            else if (v.equals("diamond")) {
                basicNode.setShape(BasicNode.POLYGON);
                basicNode.setSides(4);
                basicNode.setRegular(true);
                basicNode.setRotate(45);
            } else if (v.equals("trapezium"))
                basicNode.setShape(BasicNode.TRAPEZIUM);
            else if (v.equals("invtrapezium"))
                basicNode.setShape(BasicNode.INVTRAPEZIUM);
            else if (v.equals("parallelogram")) {
                basicNode.setShape(BasicNode.POLYGON);
                basicNode.setSides(4);
                basicNode.setSkew(0.6);
            } else if (v.equals("house"))
                basicNode.setShape(BasicNode.HOUSE);
            else if (v.equals("invhouse"))
                basicNode.setShape(BasicNode.INVHOUSE);
            else if (v.endsWith("octagon")) {
                basicNode.setShape(BasicNode.POLYGON);
                basicNode.setSides(8);
                if (v.equals("doubleoctagon"))
                    basicNode.setPeripheries(2);
                else if (v.equals("tripleoctagon"))
                    basicNode.setPeripheries(3);
                else basicNode.setPeripheries(1);
            } else if (v.endsWith("agon")) {
                basicNode.setShape(BasicNode.POLYGON);
                if (v.startsWith("pent"))
                    basicNode.setSides(5);
                if (v.startsWith("hex"))
                    basicNode.setSides(6);
                if (v.startsWith("sept"))
                    basicNode.setSides(7);
            } else
